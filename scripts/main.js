document.addEventListener('DOMContentLoaded', () => {
  ...
  // GitHub stats: fetch from public/stats.json (generated by GitHub Actions)
  ...
  function animateCount(el, to, isFormatted = true) {
    const start = 0;
    const duration = 1200;
    ...
    function tick(now) {
      ...
      el.textContent = isFormatted ? formatNumber(value) : value;
      if (progress < 1) requestAnimationFrame(tick);
      else el.textContent = isFormatted ? formatNumber(to) : to;
    }
  ...
  async function loadStats() {
    try {
      const res = await fetch('public/stats.json');
      ...
      if (commitsEl && stats.commits) animateCount(commitsEl, stats.commits);
      if (linesEl && stats.lines) animateCount(linesEl, stats.lines, true);
      if (reposEl && stats.repos) animateCount(reposEl, stats.repos);
      if (prsEl && stats.prs) animateCount(prsEl, stats.prs);
    } catch (err) {
